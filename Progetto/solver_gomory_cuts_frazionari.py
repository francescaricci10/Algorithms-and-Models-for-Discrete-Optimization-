...
def solve_problem_frazionario(instanceProblem, iteration_manager):
    ...
    if create_cut_constraint:
        ...
        cut_constraint_coefficients[cut_constraint_index] = 0

        # ora che abbiamo tutte le parti frazionarie dei coefficienti, possiamo aggiungerle al modello
        ...
        # si aggiunge al vettore b dei termini noti il nuovo termine noto dato dalla parte frazionaria del valore ottimo
        # della variabile del vincolo selezionato per il taglio
        if optimal_values_basis_variables[cut_constraint_index] >= 0:
            new_b_vector[0] = math.modf(optimal_values_basis_variables[cut_constraint_index])[0]
        else:
            new_b_vector[0] = math.fabs(math.floor(optimal_values_basis_variables[cut_constraint_index])) \
