# solver con tagli di Gomory frazionari del metodo del simplesso per il Set Covering Problem

import cplex
import math


def solve_problem_multiple_frazionario(instanceProblem, iteration_manager):
    ...
    if create_cut_constraint:
        ...
        for index in range(len(cut_constraint_index_list)):
            ...
            for j in range(num_variables):
                if cut_constraint_coefficients[j] >= 0:
                    double_part = math.modf(cut_constraint_coefficients[j])[0]
                else:
                    double_part = math.fabs(math.floor(cut_constraint_coefficients[j])) + cut_constraint_coefficients[j]
                cut_constraint_coefficients[j] = double_part

            # si aggiunge alla matrice il nuovo vincolo di cutting plane
            new_matrix[index] = cut_constraint_coefficients

            # si aggiunge al vettore b dei termini noti il nuovo termine noto dato dalla parte frazionaria del valore
            # ottimo della variabile del vincolo selezionato per il taglio
            if optimal_values_basis_variables[cut_constraint_index_list[index]] >= 0:
                new_b_vector[index] = math.modf(optimal_values_basis_variables[cut_constraint_index_list[index]])[0]
            else:
                new_b_vector[index] = math.fabs(math.floor(optimal_values_basis_variables[cut_constraint_index_list[index]])) \
                                  + optimal_values_basis_variables[cut_constraint_index_list[index]]

            # poniamo a 0 il valore del coefficiente della variabile corrispondente al dizionario scelto come taglio
            current_cut_variable_index = cut_constraint_index_list[index]
            cut_constraint_coefficients[basis_variables_indexes[current_cut_variable_index]] = 0
